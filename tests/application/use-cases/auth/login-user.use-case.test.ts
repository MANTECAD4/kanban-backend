...
import { loginData, mockUserEntity } from "../../../fixtures/use-cases";
import {
  CustomError,
  ErrorCodes,
  ...
  describe("Negative cases", () => {
    ...
    describe("Positive cases", () => {
      test(`should return an access token, a refresh token and user's data`, async () => {
        ...
        expect(getByEmailSPy).toHaveBeenCalledWith(loginData.email);
    ...
    describe("Negative cases", () => {
      test(`should throw an error if there isn't an user registered with given email`, async () => {
        ...
        await expect(
          async () => await loginUseCase.execute(loginData),
